...
- **Tools panel** — CLI-parity in the browser: hybrid corpus search, similarity search, multi-paper synthesis, topic-cluster browser. Corpus-growing tools were split out into their own top-bar **🌱 Corpus ▾** dropdown (six preview-and-select expansion flows — Enrich / Expand citations / Expand by author / Inbound cites / Topic search / Coauthors — plus Cleanup-downloads that reclaims disk from already-ingested duplicates in one click, and a Pending-downloads manager)
...
| Clean up duplicate downloads | **🌱 Corpus ▾** → 🧹 Cleanup downloads |
