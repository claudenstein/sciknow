...
- **Web reader** — browser-based authoring with a full-width top bar that's organised around four dropdowns (Plan + Dashboard stay direct for high-frequency access): **📖 Book ▾** (Corkboard / History / Snapshot / Export / Settings) · **🔍 Explore ▾** (Ask Corpus / Wiki Query / Browse Papers) · **📊 Visualize ▾** (Knowledge Graph + the six ECharts tabs) · **🛠 Manage ▾** (Tools / Setup / Projects). Separated from the per-chapter writing toolbar (Edit, Autowrite, Write, Review, Revise, Verify, Insert Citations, Adversarial, Edge cases, Scores, Argue, Gaps, Bundles, Read). Live LLM streaming, corkboard view, chapter reader, argument maps, citation popovers, snapshots, version diffs
