...
def _build_monitor_layout(snap: dict, *, days: int, watch: int):
    ...
    # missing.  Shows the total + the most-affected collection for
    # context; detail lives in snap['qdrant_indexes'].
    if qdrant_indexes.get("collections"):
        missing_n = qdrant_indexes.get("missing_total", 0) or 0
        n_colls = len(qdrant_indexes.get("collections") or [])
        right_tbl.add_row("")
        row = Table.grid(padding=(0, 1), expand=True)
        row.add_column(ratio=3, style=C_DIM)
        row.add_column(justify="right", ratio=1)
        if missing_n == 0:
            row.add_row(
                "payload indexes",
                Text(f"{n_colls}/{n_colls} ✓", style=C_OK),
            )
        else:
            affected = sum(
                1 for c in qdrant_indexes["collections"] if c.get("missing")
            )
            row.add_row(
                "payload indexes",
                Text(
                    f"{missing_n} missing · {affected}/{n_colls} coll",
                    style=C_ERR,
                ),
            )
        right_tbl.add_row(row)
    ...
    # Phase 54.6.294 — slow-ingest leaderboard removed from the
    # dashboard footer (2026-04-26). Was visually noisy (5 rows of
    # title + stage tag) and rarely actionable on a stable corpus —
    # the same data still ships in `snap['slow_docs']` for any
    # downstream JSON consumer that wants it. The helper function
    # `_slow_docs_leaderboard` and the snapshot key are preserved so
    # `library monitor --json` consumers don't break.
