...
- **Phase E** (web rebuild): 🟢 main CSS extracted to `static/css/sciknow.css` (`8710b7d`); 16,394-line inline `<script>` extracted to `static/js/sciknow.js` (`74c1fc7`); 3,847-line TEMPLATE extracted to `web/templates/book_reader.html` (`691f934`). Route split: 17 resource modules under `web/routes/` so far — `projects`, `feedback`, `bibliography`, `viz`, `jobs`, `backups`, `ledger`, `pending`, `reconciliations`, `system`, `tools`, `autowrite`, `export`, `catalog`, `snapshots` (`efab936`), `wiki` (`1d19573`). Pattern: lazy `from sciknow.web import app as _app` for cross-module helpers + re-export at the bottom of app.py for `getattr`/`inspect.getsource` test coverage. `web/app.py`: 29 kLOC → 6.1 kLOC (-79%). Remaining groups (book / draft / chapters / corpus / visuals) follow the same recipe.
