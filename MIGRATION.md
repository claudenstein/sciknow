...
- **Phase E** (web rebuild): 🟢 main CSS block extracted to `static/css/sciknow.css` (`8710b7d`); 16,394-line inline `<script>` extracted to `static/js/sciknow.js` (`74c1fc7`); 3,847-line TEMPLATE string extracted to `web/templates/book_reader.html` (this commit). `web/app.py`: 29 kLOC → 8.8 kLOC (-69.6%). The `web/routes/` resource-split (move route handlers out of app.py into `web/routes/{books,drafts,papers,wiki,kg,jobs,admin}.py`) is the only Phase E sub-task remaining — deferred to v2.1.
