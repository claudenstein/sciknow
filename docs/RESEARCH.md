...
6. **Five complementary corpus-expansion methods beyond the current `expand` / `expand-author` / `enrich` trio.** The current set is outbound-citation (`expand`), author-by-name (`expand-author`), and missing-DOI (`enrich`). These cover three orthogonal growth vectors but miss several more. Ranked by signal/effort ratio:

   **a. Inbound-citation discovery (`db expand-cites`)** — For each DOI in the corpus, query OpenAlex `/works?filter=cites:{work_id}` to enumerate papers that cite it, union across the corpus, dedup, run through the same Phase-49 RRF ranker, download + ingest. Mirror of `expand`'s outbound path; catches forward-in-time work from researchers who aren't yet in the corpus. Small effort (one OpenAlex client method, ~200 LoC, reuses the existing ranker and download pipeline). Main risk: API load scales linearly with corpus size; needs per-seed hard cap + one-timer filter. **High priority.**

   **b. Topic-driven broad search (`db expand-topic`)** — Free-text query (e.g. "thermospheric cooling") against OpenAlex `/works?search=` sorted by citation count, paginated to ~500–2k, unioned with arXiv subject/topic listings, deduped against corpus, relevance-filtered (centroid OR the query itself as the anchor), ranked, downloaded. Push-based rather than pull-based — the only existing method that solves the **bootstrap problem** (corpus too small to have references worth expanding) and the sideways-expansion problem (related field not yet represented). Small effort, reuses ranker and download. Main risk: query ambiguity ("cloud" → physics or distributed systems); mitigate with a `--dry-run` preview + higher-than-default threshold (0.65 vs 0.55). **Medium priority** — powerful but needs the user to know what to search for.

   **c. Scheduled velocity watcher (`watch add-velocity`)** — Extend the existing `watch` infrastructure (currently repo/benchmark monitoring) with a semantic-query watcher: on each `watch check`, OpenAlex is queried for papers published in the last N days matching the query, ranked by `citation_count / years_since_pub` (already computed as a Phase-49 signal). Delta from last check surfaces new-and-hot papers. Optional `--auto-ingest` for the top-K. Solves the "researcher forgets to re-expand quarterly" problem. Small-medium effort (~300 LoC, reuses watch + OpenAlex). Main risk: velocity is noisy on short time windows; always require curator review before ingest by default. **High priority** — keeps the corpus fresh with minimal user overhead.

   **d. Gap-driven auto-expansion (`book auto-expand`)** — Pipe `book gaps` output (already written to the `book_gaps` table) into `expand-topic` automatically: for each identified gap, synthesize a search query, run `expand-topic --dry-run` with the gap title as the relevance anchor, stage the shortlist for curator review. Closes the loop between "what's missing from my book" and "what to ingest next" without manual query construction. Small effort (~150 LoC of orchestration, once `expand-topic` exists). Main risk: LLM-generated queries drift; show the generated query, allow override. **Medium priority** — depends on `expand-topic` being in place first.

   **e. Coauthor network snowball (`db expand-coauthors`)** — Extract all author IDs from the corpus via OpenAlex `paper.authorships.author.id`, fetch all papers by those authors at depth=1 (or depth=2 with strict per-author caps), dedup and relevance-filter. Captures the **invisible college** — researchers in the same lab who write similar papers without citing each other directly. Medium effort (~400 LoC; needs author-dedup via canonical OpenAlex IDs, pagination, rate-limit management). Main risk: noise explosion at depth=2 on large corpora; mitigate with strict depth cap (≤2), per-author paper limit (≤10), mandatory relevance filter, one-timer filter. **Medium priority** — high-signal in tight research communities, lower in broad fields.

   **Shipping order.** a → c → b → d → e. Rationale: a and c are highest-signal with lowest implementation risk (direct reuse of existing infrastructure). b and d are user-driven and unblock bootstrap + gap-closure. e is powerful but needs careful noise gating and should go last.

   **Cross-cutting UX.** All five should expose the same preview-and-select flow introduced for `expand-author` (Phase 54.6.1) and `expand` (Phase 54.6.3) — checkboxed shortlist with per-row scores, select-by-threshold, "Download selected". The `db download-dois` primitive is already in place to back the selected-download step for any method; new endpoints only need to deliver the shortlist JSON.
