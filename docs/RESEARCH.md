...
3. **Mention-paragraph alignment** — cross-encoder score between *s* and the original mention-paragraph that cited this figure in the source paper. Per SciCap+ this text is the strongest retrieval signal against author intent. **Requires new infrastructure**: the MinerU content_list currently loses `\ref{fig:X}`–paragraph linkage. A chunker pass that resolves `Fig. N` / `Figure N` mentions in body text to `figure_num` payloads on visuals would unlock this. Moderate build cost; high expected value.
