...
6. **Five complementary corpus-expansion methods beyond the current `expand` / `expand-author` / `enrich` trio.** The current set is outbound-citation (`expand`), author-by-name (`expand-author`), and missing-DOI (`enrich`). These cover three orthogonal growth vectors but miss several more. Three of the five have **shipped (Phase 54.6.4)**; two remain deferred.
   ...
   **c. Scheduled velocity watcher (`watch add-velocity`) — DEFERRED.** Would extend the existing `watch` infrastructure (currently repo/benchmark monitoring) with a semantic-query watcher: on each `watch check`, OpenAlex is queried for papers published in the last N days matching the query, ranked by `citation_count / years_since_pub` (already computed as a Phase-49 signal). Delta from last check surfaces new-and-hot papers; optional `--auto-ingest` for the top-K. Solves the "researcher forgets to re-expand quarterly" problem. Deferred because it's the only method here that's scheduled rather than interactive, and it depends on real cron / systemd-timer integration rather than a one-shot trigger; want to ship the interactive methods first and see which deserves to become periodic.

   **d. Gap-driven auto-expansion (`book auto-expand`) — DEFERRED.** Would pipe `book gaps` output (already written to the `book_gaps` table) into `expand-topic` automatically: for each identified gap, synthesize a search query, run `expand-topic --dry-run` with the gap title as the relevance anchor, stage the shortlist for curator review. Closes the loop between "what's missing from my book" and "what to ingest next" without manual query construction. Deferred because it's composition-only on top of `expand-topic` (~150 LoC of orchestration) — easier to judge its value after we see how manual `expand-topic` is used.
