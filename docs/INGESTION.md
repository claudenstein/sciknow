...
1. **MinerU 2.5-Pro VLM** (primary backend post-54.6.212, roadmap 3.1.6) — A 1.2B Qwen2VL model fine-tuned on MinerU's data engine, scoring **95.69 on [OmniDocBench v1.6](https://arxiv.org/abs/2604.04771)** — current SOTA among open-source PDF parsers. Ships four capabilities that the pipeline backend lacks: **cross-page table merging**, **truncated paragraph merging** (no more "…conti-" / "nued…" breaks across a page boundary), **in-table image recognition**, **image/chart parsing** with structured per-figure output. Inference backend chosen via `MINERU_VLM_BACKEND` (`auto` | `transformers` | `vllm`, default `vllm`). Model identifier (default `opendatalab/MinerU2.5-Pro-2604-1.2B`) overridable via `MINERU_VLM_MODEL`. Requires `mineru[vllm]` extras (`uv add 'mineru[vllm]'`) and a GPU with ~4 GB free VRAM. On a 3090 with vLLM, runs roughly 2-4× faster than the transformers path.

2. **MinerU 2.5 pipeline** (fallback, **deprecated as a pinned backend**) — OpenDataLab's legacy pipeline: DocLayout-YOLO layout → MFD+MFR formulas → table OCR + structure reconstruction → text OCR → seal detection. Scores 86.2 on [OmniDocBench v1.5](https://arxiv.org/abs/2412.07626). Retained as the automatic fallback when VLM-Pro extras aren't installed, but `PDF_CONVERTER_BACKEND=mineru` explicitly pinned emits a one-shot `DeprecationWarning` (see 54.6.212).

3. **Marker JSON** (fallback) — `marker-pdf`'s `JSONRenderer` produces a structured block tree (`SectionHeader`, `Text`, `Table`, `Equation`, `ListItem`, ...). Used automatically when both MinerU backends fail (rare — typically pure scans with no embedded text), or when `PDF_CONVERTER_BACKEND=marker`.
